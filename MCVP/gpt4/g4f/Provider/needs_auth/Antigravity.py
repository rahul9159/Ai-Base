...
class AntigravityAuthManager(AuthFileMixin):
    ...
    # Provide OAuth app credentials via environment variables.
    OAUTH_CLIENT_ID_ENV = "ANTIGRAVITY_OAUTH_CLIENT_ID"
    OAUTH_CLIENT_SECRET_ENV = "ANTIGRAVITY_OAUTH_CLIENT_SECRET"
    ...
    @classmethod
    def get_oauth_client_credentials(cls) -> Tuple[str, str]:
        """Load OAuth client credentials from environment variables."""
        client_id = os.getenv(cls.OAUTH_CLIENT_ID_ENV, "").strip()
        client_secret = os.getenv(cls.OAUTH_CLIENT_SECRET_ENV, "").strip()
        if not client_id or not client_secret:
            raise RuntimeError(
                f"Missing Antigravity OAuth client credentials. "
                f"Set {cls.OAUTH_CLIENT_ID_ENV} and {cls.OAUTH_CLIENT_SECRET_ENV}."
            )
    ...
    def build_authorization_url(cls, project_id: str = "") -> Tuple[str, str, str]:
        ...
        """
        client_id, _ = cls.get_oauth_client_credentials()
    ...
    async def exchange_code_for_tokens(
        ...
        """
        client_id, client_secret = cls.get_oauth_client_credentials()
